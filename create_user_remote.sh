...
# Need this in case user has a provider that offers private IP's and doesnt re-run the yiimpool installer after user creation
...
ARCHITECTURE=$(uname -m)
  if [ "$ARCHITECTURE" != "x86_64" ]; then
    if [ -z "$ARM" ]; then
      echo "Afiniel Yiimp Multi Server Installer v4.7.0 only supports x86_64 and will not work on any other architecture, like ARM or 32 bit OS."
